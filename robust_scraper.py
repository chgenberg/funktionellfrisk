#!/usr/bin/env python3
"""
Robust skrapare som fortsätter även om några sajter misslyckas
"""
import requests
...
class RobustHealthSiteScraper:
    ...
    def scrape_site_info(self, domain: str) -> Dict:
        """Skrapar detaljerad information från en hälsosajt med robust felhantering"""
        url = f"https://{domain}"
        
        try:
            print(f"🔄 Skrapar {domain}...")
            
            # Kortare timeout för att undvika att fastna
            ...
            print(f"✅ Skrapade {domain} ({response.elapsed.total_seconds():.2f}s)")
            ...
            print(f"⏰ Timeout för {domain} - hoppar över")
            return {'domain': domain, 'error': 'timeout'}
        except requests.exceptions.ConnectionError as e:
            print(f"🔌 Anslutningsfel för {domain} - hoppar över")
            return {'domain': domain, 'error': f'connection_error: {str(e)[:100]}'}
        except requests.exceptions.HTTPError as e:
            print(f"🚫 HTTP-fel för {domain}: {e.response.status_code}")
            return {'domain': domain, 'error': f'http_error: {e.response.status_code}'}
        except Exception as e:
            print(f"❌ Okänt fel vid skrapning av {domain}: {str(e)[:100]}")
            return {'domain': domain, 'error': f'unknown_error: {str(e)[:100]}'}
...
def main():
    """Huvudfunktion för att skrapa alla sajter robust"""
    scraper = RobustHealthSiteScraper()
    ...
    domains = [
        ...
        'teresealven.se',
                 'paleoteket.se',
         'medvetenandning.se',
         # 'mindfulnesscenter.se',  # BORTTAGEN - fastnade här
         'funmed.se',
        'ehdin.com',
        'martinajohansson.se',
        'coach4lifesweden.se',
    ...
    print(f"🚀 Startar robust skrapning av {len(domains)} sajter...")
    print("💡 Denna version hoppar över problematiska sajter och fortsätter!")
    ...
    for i, domain in enumerate(domains, 1):
        ...
        # Spara progress varje 10:e sajt
        if i % 10 == 0:
            with open('scraped_site_data_progress.json', 'w', encoding='utf-8') as f:
                json.dump(scraped_data, f, ensure_ascii=False, indent=2)
            print(f"\n📊 Progress: {successful} lyckade, {failed} misslyckade av {i} totalt")
    ...
    print(f"\n🎉 Skrapning klar!")
    print(f"✅ {successful} sajter skrapade framgångsrikt")
    print(f"❌ {failed} sajter misslyckades")
    print(f"📁 Data sparad i 'scraped_site_data.json'")
